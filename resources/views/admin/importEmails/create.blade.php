...
@section('page-header')
	...
	<div class="page-header mt-5-7"> 
		<div class="page-leftheader">
			<h4 class="page-title mb-0">{{ __('New Email') }}</h4>
			<ol class="breadcrumb mb-2">
				<li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}"><i class="fa-solid fa-message-exclamation mr-2 fs-12">
					
				</i>{{ __('Admin') }}</a></li>
				<li class="breadcrumb-item" aria-current="page"><a href="{{ route('admin.emails') }}"> {{ __('Emails') }}</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="{{url('#')}}"> {{ __('New Email') }}</a></li>
			</ol>
		</div>
	</div>
	<!-- END PAGE HEADER -->
		...
		<div class="col-lg-6 col-md-6 col-xm-12">
			<div class="card overflow-hidden border-0">
				<div class="card-header">
					<h3 class="card-title">{{ __('Create New Email') }}</h3>
				</div>
				<div class="card-body pt-5">									
					<form action="{{ route('admin.email.send') }}" method="POST" enctype="multipart/form-data">
						...
						<div class="border-0 text-right mb-2 mt-1">
							<a href="{{ route('admin.emails') }}" class="btn btn-cancel mr-2">{{ __('Return') }}</a>
