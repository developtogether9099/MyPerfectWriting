...
<div class="page-header mt-5-7">
    <div class="page-leftheader">
        <h4 class="page-title mb-0">{{ __('All Paid Orders') }}</h4>
        <ol class="breadcrumb mb-2">
            <li class="breadcrumb-item"><a href="{{ route('user.dashboard') }}"><i class="fa-solid fa-id-badge mr-2 fs-12"></i>{{ __('Dashboard') }}</a></li>
            <li class="breadcrumb-item" aria-current="page"><a href="{{ route('user.my_orders') }}">{{ __('Orders List') }}</a></li>
        </ol>
    </div>
...
<a href="{{route('user.my_inProcess_orders')}}" class="bg-dark">
...
<a href="{{route('user.my_requestedForRevision_orders')}}" class="bg-dark">
...
<a href="{{route('user.my_completed_orders')}}" class="bg-dark">
...
@section('js')
	...
	<script type="text/javascript">
		$(function () {
			...
			var url = "{{ route('user.my_orders') }}";
			//alert(value)
			if(value == 'my_orders'){
				url = "{{ route('user.my_orders') }}"
			} else if(value == 'my_completed_orders') {
				url = "{{ route('user.my_completed_orders') }}"
			} else if(value == 'my_inProcess_orders') {
				url = "{{ route('user.my_inProcess_orders') }}"
			} else if(value == 'my_inProcess_orders') {
				url = "{{ route('user.my_inProcess_orders') }}"
			}  else if(value == 'my_requestedForRevision_orders') {
				url = "{{ route('user.my_requestedForRevision_orders') }}"
			}
