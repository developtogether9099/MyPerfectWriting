...
class UserDashboardController extends Controller
{
    ...
    {                         
		...
		$orders = Order::where('customer_id', auth()->user()->id)->where('payment_status',1)->orderBy('id','DESC')->limit(5)->get(); $total_commission = Referral::select(DB::raw("sum(commission) as data"))->where('referrer_id', auth()->user()->id)->get();
		
        return view('user.dashboard.index', compact('total_commission','data', 'chart_data', 'template_quantity', 'templates', 'subscription', 'custom_templates', 'chat_quantity', 'favorite_chats','orders'));           
    }
